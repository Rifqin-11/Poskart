...
export function GalleryBrandingPage() {
  ...
  const save = async () => {
    ...
    try {
      ...
      toast.success(t("themes.galleryBrandingSaved"));
    } catch (error) {
      showErrorToast(
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[minmax(280px,0.75fr)_minmax(520px,1.25fr)]">
      <section className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
        <div className="mb-5">
            <h2 className="text-base font-semibold text-zinc-900">{t("themes.galleryBranding")}</h2>
        ...
        <div className="space-y-4">
          ...
          <div>
            <span className="block text-xs font-medium text-zinc-600">{t("themes.businessLogo")}</span>
            <div className="mt-1 flex items-center gap-3 rounded-xl border border-zinc-200 p-3">
              ...
              <label className="flex-1 cursor-pointer rounded-lg border border-dashed border-zinc-300 px-3 py-2 text-center text-xs font-medium text-zinc-600 hover:bg-zinc-50">
                ...
                {uploading ? t("themes.uploading") : t("themes.uploadLogo")}
              ...
              <button type="button" className="text-xs text-zinc-400 hover:text-zinc-700" onClick={() => update({ logoUrl: DEFAULT_GALLERY_BRANDING.logoUrl })}>{t("themes.resetDefault")}</button>
            </div>
          </div>
          ...
          <label className="block text-xs font-medium text-zinc-600">
            ...
            <span className="mt-1 block text-[10px] font-normal text-zinc-400">{t("themes.copyrightHint")}</span>
          </label>
          <div className="flex items-center justify-between gap-2 border-t border-zinc-100 pt-4">
            <Button type="button" variant="ghost" onClick={reset}><RotateCcw className="mr-2 size-3.5" />{t("themes.resetDefault")}</Button>
            <Button type="button" disabled={uploading || updateBranding.isPending} onClick={() => void save()}>{updateBranding.isPending ? "Menyimpan..." : "Simpan branding"}</Button>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-zinc-200 bg-zinc-100 p-4 shadow-sm sm:p-6">
        <div className="mb-4 flex items-center justify-between">
          <div>
           ...
           <span className="rounded-full bg-white px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-zinc-500">{t("themes.publicGallery")}</span>
        </div>
