...
const workflow = [
  "Buat akun dan pilih paket sesuai jumlah perangkat.",
  "Atur identitas brand, aset, dan template booth.",
  "Hubungkan perangkat lalu terbitkan konfigurasi.",
  "Pantau transaksi dan operasi dari dashboard.",
];
  ...
  return (
    <main className="min-h-screen overflow-clip bg-[#fbfaf8] text-zinc-950">
      ...
      <section className="relative isolate min-h-[calc(100svh-4rem)] overflow-hidden border-b border-zinc-200">
        ...
        <div className="absolute inset-0 -z-10 bg-[linear-gradient(to_right,rgba(24,24,27,0.05)_1px,transparent_1px),linear-gradient(to_bottom,rgba(24,24,27,0.05)_1px,transparent_1px)] bg-[size:64px_64px] [mask-image:linear-gradient(to_bottom,black,transparent_85%)]" />
        ...
        <div className="mx-auto grid min-h-[calc(100svh-4rem)] max-w-7xl items-center gap-12 px-4 py-16 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:px-8">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-rose-200 bg-white/70 px-3 py-1.5 text-xs font-medium text-rose-700 shadow-sm backdrop-blur">
              <Sparkles className="size-3.5" />
              Operating system untuk photobooth modern
            </div>
            <h1 className="mt-7 max-w-4xl text-balance text-5xl font-semibold tracking-[-0.055em] sm:text-6xl lg:text-[5.25rem] lg:leading-[0.95]">
            ...
            <div className="mt-9 flex flex-col gap-3 sm:flex-row">
              <Link
                ...
                className={buttonVariants({
                  size: "lg",
                  className:
                    "h-12 bg-zinc-950 px-6 text-white shadow-xl shadow-zinc-950/15 hover:bg-zinc-800",
                })}
              >
              ...
              <Link
                ...
                className={buttonVariants({
                  variant: "outline",
                  size: "lg",
                  className: "h-12 border-zinc-300 bg-white/70 px-6",
                })}
              >
                Lihat paket
            ...
            <a
              href="#platform"
              className="mt-12 inline-flex items-center gap-2 text-sm font-medium text-zinc-500 transition-colors hover:text-zinc-950"
            >
          ...
          <div className="relative mx-auto w-full max-w-xl">
            ...
            <div className="rotate-1 rounded-[2rem] border border-zinc-200/80 bg-white/80 p-3 shadow-2xl shadow-rose-950/10 backdrop-blur-xl">
            ...
            {capabilities.map((capability) => {
              ...
              return (
                <article
                  ...
                  className="group bg-white p-6 transition-colors hover:bg-rose-50/50 sm:p-8"
                >
      ...
      <section className="border-y border-zinc-200 bg-white">
        <div className="mx-auto grid max-w-7xl gap-12 px-4 py-24 sm:px-6 lg:grid-cols-[1fr_0.9fr] lg:px-8">
          <div>
          ...
          <ol className="space-y-3">
            {workflow.map((item, index) => (
              <li
                key={item}
                className="flex gap-4 rounded-2xl border border-zinc-200 bg-[#fbfaf8] p-5"
              >
                <span className="grid size-9 shrink-0 place-items-center rounded-full bg-zinc-950 font-mono text-xs text-white">
                  {String(index + 1).padStart(2, "0")}
                </span>
                <p className="pt-1.5 text-sm leading-6 text-zinc-700">{item}</p>
              </li>
            ))}
          </ol>
        </div>
