alter table public.queue_events
  add column if not exists device_id text references public.devices(id) on delete set null;

create index if not exists queue_events_device_idx
  on public.queue_events (organization_id, device_id, status);
