-- Migration: Organization onboarding with unique join codes
...
set join_code = public.generate_organization_join_code()
where join_code is null;

alter table public.organizations
  alter column join_code set not null;

-- New users should create or join an organization from onboarding, not receive
-- an automatic workspace immediately after registration.
...
begin
  ...
  insert into public.organization_members (organization_id, profile_id, role)
  values (target_org.id, current_user_id, 'staff');
